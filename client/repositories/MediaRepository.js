...
class MediaRespository {
  async getBannersBySlugs(payload) {
    ...
    const reponse = await Repository.get(`${baseUrl}/api/banners?${query}`)
      .then((response) => {
  ...
  async getBannersBySlug(payload) {
    const endPoint = `api/banners?slug_in=${payload}`
  ...
  async getPromotionsBySlug(payload) {
    const endPoint = `api/promotions?slug_in=${payload}`
    const reponse = await Repository.get(`${baseUrl}/${endPoint}`)
      .then((response) => {
        if (response.data) {
          return response.data[0].items
  ...
  async getPromotionsBySlugs(payload) {
    ...
    const reponse = await Repository.get(`${baseUrl}/api/promotions?${query}`)
      .then((response) => {
        return response.data
      })
      .catch((error) => ({
