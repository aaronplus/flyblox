...
const ProductGroupByCarousel = ({ filter, title, layout = 'standard' }) => {
  const sliderRef = useRef(null)
  const { productItems, loading, getProductsByPagination } = useGetProducts()
    ...
    <div className='ps-block--shop-features'>
      <div className='ps-block__header'>
        ...
        <div className='ps-block__navigation'>
          <a className='ps-carousel__prev' onClick={e => handleCarouselPrev(e)}>
            <i className='icon-chevron-left'></i>
          </a>
          <a className='ps-carousel__next' onClick={e => handleCarouselNext(e)}>
            <i className='icon-chevron-right'></i>
          </a>
        </div>
      </div>
      {productItems && productItems.length > 4 && (
